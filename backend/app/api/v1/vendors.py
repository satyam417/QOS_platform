...
from app.schemas.kyc import KYCSubmission, KYCStatusResponse
...
def create_vendor_profile(
    ...
    existing = db.scalar(select(VendorProfile).where(VendorProfile.user_id == current_user.id))
    if existing:
...
def get_vendor_by_id(
    ...
    profile = db.scalar(select(VendorProfile).where(VendorProfile.id == vendor_id))
    if not profile:
